...
TEST_CASE("Testa a leitura de uma imagem em formato PPM") {
    ...
    const Cor& p1 = img(0, 0);
    CHECK(p1.r == 255);
    CHECK(p1.g == 0);
    CHECK(p1.b == 0);

    const Cor& p2 = img(1, 0); // (x,y) = (1,0)
    CHECK(p2.r == 0);
    CHECK(p2.g == 255);
    CHECK(p2.b == 0);

    const Cor& p3 = img(0, 1); // (x,y) = (0,1)
    CHECK(p3.r == 255);
    CHECK(p3.g == 255);
    CHECK(p3.b == 0);

    const Cor& p4 = img(1, 1);
    CHECK(p4.r == 255);
    CHECK(p4.g == 255);
    CHECK(p4.b == 255);
}
